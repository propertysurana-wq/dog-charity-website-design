...
const Footer: React.FC = () => {
  ...
  const footerLinks = {
    ...
    'Our Work': [
      ...
      { label: 'Problems We Resolved',  path: '/our-work/dog-problem-resolution' },
    ],
    // Navbar: standalone pages
    'Support & Care': [
      { label: 'Shelter & Safe Care', path: '/shelter-safe-care' },
      { label: 'Become a Volunteer',  path: '/get-involved' },
      { label: 'Donate',              path: '/donate' },
      ...
      <div className="max-w-7xl mx-auto px-4 py-12">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
          ...
          <div className="lg:col-span-1">
            ...
            <div className="space-y-2 text-sm text-gray-300">
              {/* <a href={`tel:${orgInfo.phone}`} className="flex items-center gap-2 hover:text-[#FFD100] transition-colors">
                <Phone className="w-4 h-4" />
                {orgInfo.phone}
              </a> */}
              <a href={`mailto:${orgInfo.email}`} className="flex items-center gap-2 hover:text-[#FFD100] transition-colors">
                <Mail className="w-4 h-4" />
                {orgInfo.email}
              </a>
              <p className="flex items-start gap-2">
          ...
          {/* Footer Links — same pages as Navbar */}
          {Object.entries(footerLinks).map(([title, links]) => (
            <div key={title}>
              <h4 className="font-bold text-lg mb-4 text-[#FFD100]">{title}</h4>
              <ul className="space-y-2">
                {links.map((link) => (
                  <li key={link.label}>
                    <Link
        ...
        <div className="border-t border-gray-700 mt-8 pt-8">
          <div className="flex flex-col md:flex-row items-center justify-between gap-4">
            ...
            <div className="text-sm text-gray-400 text-center md:text-right">
              <p>© {currentYear} {orgInfo.name}. All rights reserved.</p>
              <p className="mt-1">
                <Link to="/privacy" className="hover:text-[#FFD100]">Privacy Policy</Link>
                {' · '}
                <Link to="/terms" className="hover:text-[#FFD100]">Terms of Use</Link>
