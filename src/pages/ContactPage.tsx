...
const ContactPage: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch('https://dogbackend-coral.vercel.app/api/contact', {
        method: 'POST',
              ...
              {formSubmitted ? (
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                  ...
                  <button
                    type="submit"
                    disabled={loading}
                    className="inline-flex items-center gap-2 px-8 py-4 bg-[#002B5C] text-white font-bold text-base rounded-full border-none cursor-pointer transition-all hover:bg-[#003d85] hover:translate-y-[-2px] disabled:opacity-70 disabled:cursor-not-allowed disabled:transform-none"
                  >
                    {loading ? (
                      <>
